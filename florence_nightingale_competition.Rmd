---
title: "**LA GUERRA DE FLORENCE NIGHTINGALE**"
author: "Laura Navarro Soler  y  Xavier Vivancos García"
date: '`r Sys.Date()`'
output:
  html_document:
    toc: yes
...
<center><img
src="https://www.branchcollective.org/wp-content/uploads/2016/08/2Nightingale-illustrated-london-news-feb-24-1855.jpg" >
</center>

# **Presentación**

* **Laura Navarro Soler**. Periodista de datos, graduada en Periodismo por la Universidad Miguel Hernández de Elche (2017) y posgrado en Periodismo de Datos y Visualización por la Universitat Blanquerna (2019). Trabajo como _freelance_ con interés en temas sociales, ambientales y científicos.

* **Xavier Vivancos García**. Ingeniero de Telecomunicaciones por la Universitat Pompeu Fabra y Máster en Business Intelligence y Big Data (Universitat Oberta de Catalunya). Actualmente trabajando como consultor BI, utilizando mayormente Power BI para elaborar y mantener _dashboards_. Apasionado de R y Kaggle.  
...
```{r fig.align='center', message=FALSE, warning=FALSE, dpi=300, fig.width=9, fig.height=7}
# Librerías
...
tree <- treemap(datos,
                ...
                fontsize.labels = c(11,0),
                ymod.labels = 0.1,
                aspRatio = 4/1
)
```

Cuando en noviembre de 1854 Florence llegó a Scutari -la base de operaciones británicas- junto con un equipo de 38 enfermeras voluntarias, las muertes por enfermedades casi triplicaban a las de heridas de guerra, en ese momento fruto de la batalla que se estaba desarrollando en Inkerman. 
...
acum_p <- ggplot(muertes, aes( x = date, y = muertos, fill = causa_muerte)) +
  ...
  theme_classic() +
  labs(title="Muertes de los soldados británicos",
  ...
  annotate(geom = "text", x = as.Date("1855-03-01"), y = 6000, 
           ...
           angle = 90, size = 2.8, vjust = -0.5, hjust = 0.3) +
  ...
  annotate(geom = "text", x = as.Date("1855-09-01"), y = 6000, 
           ...
           angle = 90, size = 2.8, vjust = -0.5, hjust = 0.1) +
  annotate(geom = "text", x = as.Date("1855-03-01"), y = 13550, 
           label = "El 85% muere por\n enf. zimóticas", color = "black",
           size = 3, vjust = -0.5, hjust = 0.5) +
  annotate(geom = "text", x = as.Date("1855-09-01"), y = 17000, 
           label = "El 39% muere por\n enf. zimóticas", color = "black",
           size = 3, vjust = -0.5, hjust = 0.5)
acum_p
```

```{r fig.align='center', fig.height=7, fig.width=9.5, dpi=300, message=FALSE, warning=FALSE}
# Lectura datos
data <- read.xlsx("C:/Users/xviva/OneDrive/Desktop/datos_florence.xlsx", 1) %>%
  rename(`Average size of army`=Average.size.of.army,
         `Enfermedades zimóticas`=Zymotic.diseases,
         `Heridas de guerra`=Wounds...injuries,
         `Otras`=All.other.causes,
         `Zymotic diseases (Annual rate)`=Zymotic.diseases.1,
         `Wounds & injuries (Annual rate)`=Wounds...injuries.1,
         `All other causes (Annual rate)`=All.other.causes.1) %>%
...
p <- ggplot(data=data2, aes(x=Month, fill=type)) + 
  ...
  labs(y="Muertes", title="<span style='font-size:14pt'>La guerra de Florence</span><br>
  <span style='font-size:11pt'><span style='color:firebrick;'> Incremento</span> y 
       <span style='color:darkseagreen;'>disminución</span> de muertos durante el transcurso del
  conflicto</span>") +
  ...
  geom_text(data=annotates, aes(x=x, y=y, label=label), size=2.5) +
